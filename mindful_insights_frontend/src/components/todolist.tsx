...
const Todolist = () => {
  const { accessToken } = useAuth();
    ...
    const [researchList, setResearchList] = useState<{ topic: string, abstract: string }[]>([]);

    useEffect(() => {
        const fetchResearch = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/api/research/status/approved`, {
                    headers: {
    ...
    <div className="d-flex flex-column align-items-stretch flex-shrink-0 bg-body-tertiary word-goup" style={{width: "380px"}}>
        ...
        <div className="list-group list-group-flush border-bottom scrollarea" style={{ backgroundColor: "#0074CC"}}>
          {researchList.map((research, index) => (
            <a key={index} href="#" className="list-group-item list-group-item-action active py-3 lh-sm" aria-current="true">
                <div className="d-flex w-100 align-items-center justify-content-between">
                    <strong className="mb-1">{research.topic}</strong>
                    <small>{/* Date or other relevant info */}</small>
                </div>
                <div className="col-10 mb-1 small">{research.abstract}</div>
            </a>
          ))}        
        </div>
    </div>
